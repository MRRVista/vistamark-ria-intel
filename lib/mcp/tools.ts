...
const RIA_TOOLS: ToolDef[] = [
  ...
  { name: "database_status", description: "Health and freshness of the database, with per-pipeline failure visibility: firm count, pipelineHealth (each ingest family's LATEST run — errors sorted first, with redacted/truncated errorMessage — so a failing scheduled ingest is visible instead of hidden), lastIngest (most recent run of ANY status), lastSuccessfulIngest, and latest ADV snapshot date. Covers DB-backed sources (ADV, IRS BMF, DOL 5500, IPEDS, NACUBO, SBA PPP, SEC 13F). A family absent from pipelineHealth has never created a run record — its handler is not being reached. Note: FDIC, OFR, SEC EDGAR (incl. full-text/Form D/frames), GLEIF LEI, BDC, public-pension (PPD), USAspending, Treasury DTS, the macro signals layer, and the morning market brief are live-API sources with no local ingest, so they do not appear here.", inputSchema: { type: "object", properties: {} }, handler: databaseStatus },
];
...
const MACRO_TOOLS: ToolDef[] = [
  ...
  },
  {
    ...
    description: "THE pre-market composite in one call — assembles the whole early-warning stack: macro_market_signals (22 indicators + net liquidity), treasury_daily_cash (daily TGA — the daily liquidity read), treasury_daily_flows (withheld-tax payroll signal + fiscal tape), fred_yield_curve (full curve + inversion flags), and ofr_financial_stress_index (daily systemic gauge). Returns headline bullets, trimmed per-source sections, and briefMarkdown — a compact markdown note sized for direct insertion into a morning-brief prompt or email. Each section is independently error-isolated, so one upstream hiccup degrades that section rather than the brief. Also served unauthenticated at GET /api/market-brief for cron consumption (aggregated public macro data only — FRED/Treasury/OFR). Built for Randall's pre-market brief; indicators, not predictions.",
    inputSchema: { type: "object", properties: {} },
    handler: morningMarketBrief,
  },
