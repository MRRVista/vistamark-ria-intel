...
export async function findLatestFeedUrl(
  feedType: typeof FEED_TYPES[number] = "IA_FIRM_SEC_Feed"
...
export async function runFullIngest(): Promise<{
  ...
  const run = await db
    .insert(ingestRuns)
    .values({ source: "sec_iapd", status: "running" })
